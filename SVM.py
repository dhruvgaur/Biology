...
if __name__ == "__main__":
    ...
    def test_non_linear():
        ...
        for i in range(len(X_test)):
            f.write(str(X_test[i][0]))
            f.write(str(","))
            f.write(str(X_test[i][1]))
            f.write(str("\n"))
    ...
    #test_linear()
    X_test =genfromtxt('NormalizedParkinsons', delimiter=',')
    Y_test =np.ones(len(X_test))
    X_train=[X_test[0]]
    X_train=np.delete(X_train,0,axis=0)
    #Read outlier file in index array
    with open("OutliersParkinsons", "r") as ins:
        index = []
        for line in ins:
            index.append(line)
